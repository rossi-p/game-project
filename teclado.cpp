...
ALLEGRO_DISPLAY *janela_teclado = NULL;
ALLEGRO_EVENT_QUEUE *fila_eventos_teclado = NULL;
...
int inicializar_teclado(){
    ...
    janela_teclado = al_create_display(LARGURA_TELA, ALTURA_TELA);
    if (!janela_teclado){
        error_msg("Falha ao criar janela_teclado");
        return 0;
    }

    al_set_window_title(janela_teclado, "Utilizando o Teclado");

    fonte = al_load_font("arial.ttf", 72, 0);
    if (!fonte){
        error_msg("Falha ao carregar \"arial.ttf\"");
        al_destroy_display(janela_teclado);
        return 0;
    }

    fila_eventos_teclado = al_create_event_queue();
    if (!fila_eventos_teclado){
        error_msg("Falha ao criar fila de eventos");
        al_destroy_display(janela_teclado);
        return 0;
    }

    //registra duas fontes de eventos na fila. o da janela_teclado, e do teclado
    al_register_event_source(fila_eventos_teclado, al_get_keyboard_event_source());
    al_register_event_source(fila_eventos_teclado, al_get_display_event_source(janela_teclado));
...
int teclado(void)
{
    ...
    while (!sair){
        while(!al_is_event_queue_empty(fila_eventos_teclado)){
            ALLEGRO_EVENT evento;
            //espera ate que algum evento esteja na fila
            al_wait_for_event(fila_eventos_teclado, &evento);
            ...
            //se clicou para fechar a janela_teclado
    ...
    al_destroy_display(janela_teclado);
    al_destroy_event_queue(fila_eventos_teclado);
