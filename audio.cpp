...
ALLEGRO_DISPLAY *janela_audio = NULL;
...
ALLEGRO_EVENT_QUEUE *fila_eventos_audio = NULL;
...
int inicializar_audio(){
    ...
    janela_audio = al_create_display(LARGURA_TELA, ALTURA_TELA);
    if(!janela_audio){
        error_msg("Falha ao criar janela_audio");
        al_destroy_sample(som_azul);
        al_destroy_sample(som_verm);
    ...
    if (!al_install_mouse()){
        ...
        al_destroy_display(janela_audio);
        return -1;
    }
    ...
    al_set_target_bitmap(al_get_backbuffer(janela_audio));

    fila_eventos_audio = al_create_event_queue();
    if(!fila_eventos_audio) {
    ...
    al_register_event_source(fila_eventos_audio, al_get_mouse_event_source());
    al_register_event_source(fila_eventos_audio, al_get_display_event_source(janela_audio));
...
int audio(){
    ...
    while (!sair){
        ...
        al_wait_for_event(fila_eventos_audio, &evento);
    ...
    al_destroy_bitmap(bitmap_azul);
    al_destroy_bitmap(bitmap_verm);
    al_destroy_event_queue(fila_eventos_audio);
    return 0;
}
